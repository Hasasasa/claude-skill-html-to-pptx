...
所有页数都走 Claude `Agent(...)` sub-agent 并行 dispatch；常规视觉判断交给 sub-agent，主 agent 不逐页 Read compare 图。

主 agent 只在 3 种情况下读**单页** compare 图复核：(1) finding 描述含糊 / 与源代码冲突无法定位；(2) sticky 命中前确认；(3) 用户点名反馈某一页。
...
每个 sub-agent prompt 里必须直接写入本批 compare 图绝对路径、deck HTML 路径、`lessons-learned.md` 路径；不要假设主 agent 的 Read / Grep 输出会自动传给 sub-agent。
...
- **不**常规自己 Read compare 图（findings 已经把症状写明）；三种例外读单页复核见上方"并行 audit"章节
...
Stage 5b 产出 `<out>_audit/`（compare 图 + `audit_index.json` + `audit_prompt.md`）。**不做像素 diff** — 数字对局部 bug 不敏感、给假信心，视觉判断完全交 5b 的 sub-agent。无渲染器时 5b 跳过，按上面"渲染器要求"小节 ask 用户。
...
2. 看 Stage 5b audit 结果：常规全量判断用 sub-agent findings；只有用户反馈 / 告警对应页需要人工复核时，才用 Read 看那一页 compare 图
