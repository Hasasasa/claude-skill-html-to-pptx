...
| Audit compare 图大面积字体回退（serif / mono 出现在本该是非系统字体的位置）+ 多页报 "标题与正文叠压" | PowerPoint COM `slide.Export()` 不读 pptx 内嵌 TTF，只用系统已装字体；回退字体更宽导致标题多换行、挤进下方段落，叠压是连锁假象。pptx 嵌入本身正确 | 用 `--install-user-fonts` 重跑——SKILL.md "字体安装确认" 要求第一次 convert 之前就 ask，避免走这一轮 |
