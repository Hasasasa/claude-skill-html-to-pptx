...
Agent(
  ...
  run_in_background=True,      # 必须 true，否则主 agent 干等 30-90s
...
**同一条 message 里跟着发并行准备**（不要让主 agent 干等 sub-agent 30-90s）：

```
Read(file_path="<deck>/template.html")                            # 修 finding 必读，先加载到 context
Read(file_path="<skill_dir>/references/lessons-learned.md")       # OOXML 边界查询用
Grep(pattern='class="slide |data-slide=', path="<deck>/template.html",
     output_mode="content", -n=true)                              # 每个 section 起始行号，定位修复点
```

只做"无论 findings 是什么都用得上"的准备——**不要预测 findings 然后预改 HTML**。详见 SKILL.md "等 sub-agent 时主 agent 必须并行做的事"。
